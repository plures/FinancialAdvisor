...
export class FinancialAdvisorMCPServer {
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            switch (name) {
                case 'add_account':
                    return await this.addAccount(args);
                ...
                case 'categorize_transactions':
                    return await this.categorizeTransactions(args);
                default:
...
${insights.topCategories.map((cat) => `- **${cat.category}**: $${cat.totalAmount.toFixed(2)} (${cat.percentage.toFixed(1)}%)`).join('\n')}

## Largest Expenses
${insights.largestExpenses.slice(0, 5).map((t) => `- $${Math.abs(t.amount).toFixed(2)} - ${t.description} (${t.category || 'Uncategorized'})`).join('\n')}

## Recurring Patterns
${insights.recurringPatterns.slice(0, 5).map((p) => `- **${p.merchant}**: $${p.averageAmount.toFixed(2)} avg, ${p.frequency.toFixed(1)}x/month`).join('\n')}
    `;
        return {
            content: [
                {
...
- Budget recommendations
    ...
    async categorizeTransactions(args) {
