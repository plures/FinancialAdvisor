...
export class TransactionAnalyzer {
     ...
     */
    static analyzeTransactions(transactions, timeframe) {
        let filteredTransactions = transactions;
        if (timeframe) {
            filteredTransactions = transactions.filter(t => t.date >= timeframe.start && t.date <= timeframe.end);
        }
    ...
    static categorizeTransaction(transaction) {
        ...
        if (transaction.amount > 0 && (searchText.includes('salary') ||
     ...
     */
    static getCategorySummaries(transactions) {
     ...
     */
    static findRecurringPatterns(transactions) {
     ...
     */
    static findUnusualTransactions(transactions) {
    ...
    static getLargestExpenses(transactions, count) {
        return transactions
            .filter(t => t.type === TransactionType.EXPENSE)
            .sort((a, b) => Math.abs(b.amount) - Math.abs(a.amount))
            .slice(0, count);
...
TransactionAnalyzer.CATEGORY_KEYWORDS = {
