...
import {
  AIProviderFactory,
  AIProviderManager,
} from '../../packages/ai-integration/dist/provider-manager.js';
import {
  AIProviderType,
  AIProviderConfig,
  FinancialContext,
...
describe('AI Provider Integration Tests', function () {
    ...
    transactions: [
      {
        ...
        accountId: '1',
        amount: moneyFromDecimal(-50.0, 'USD'),
      ...
      } as Transaction,
      {
        ...
        importSessionId: 'test-session',
        accountId: '1',
        amount: moneyFromDecimal(-120.0, 'USD'),
      ...
      } as Transaction,
      {
        ...
        importSessionId: 'test-session',
        accountId: '1',
        amount: moneyFromDecimal(3000.0, 'USD'),
  ...
  describe('OpenAI Provider Integration', () => {
    ...
    it('should categorize a transaction using OpenAI', async () => {
      ...
      const isValidCategory = validCategories.some(valid => categoryLower.includes(valid));

      assert.ok(isValidCategory, `Category "${category}" should be a food/dining related category`);
    });

    it('should analyze financial data with context', async () => {
      const response = await provider.query('What is my current savings rate?', sampleContext);
      ...
      const testCases = [
        {
          ...
          merchant: 'Amazon',
          expectedCategory: ['shopping', 'entertainment', 'subscription'],
        },
        {
          description: 'Shell Gas Station',
          merchant: 'Shell',
          expectedCategory: ['transportation', 'gas', 'fuel', 'auto'],
        },
        {
          description: 'Electric bill payment',
          merchant: 'PG&E',
          expectedCategory: ['utilities', 'bills'],
        },
      ];
  ...
  describe('Copilot Provider', () => {
    ...
    it('should get Copilot capabilities', () => {
      ...
      assert.strictEqual(
        ...
        false,
        'Copilot streaming not yet supported'
      );
      assert.strictEqual(
        capabilities.supportsFunction,
        false,
        'Copilot functions not yet supported'
      );
      assert.ok(capabilities.supportedFormats.includes('text'), 'Should support text format');
      assert.ok(
        capabilities.supportedFormats.includes('markdown'),
